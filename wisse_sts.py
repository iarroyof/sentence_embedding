...
if __name__ == "__main__":
    ...
    parser.add_argument("--tfidf", help = """Input file containing TFIDF
                                        pre-trained weights. If not provided,
                                        all weights will be 1.0 (pickled sklearn
                                        object).""", default = None)
    ...
    parser.add_argument("--comb", help = """Desired word vector combination for
                                        sentence representation {sum, moving,
                                            avg}.""", default = "sum")
    parser.add_argument("--suffix", nargs = '?', help = """A suffix to add to the
                                                        output file""",
                                                        default = "",
                                                        required = False)
    parser.add_argument("--pi_tfidf", help="""To predict TFIDF weights (pred) or
                                            ...
                                            1.0 (1).""", default=None)
    parser.add_argument("--ortho", help = """To orthogonalize word vectors by
                                            using the Gram?Schmidt process
                                            (ortho). (ld) keeps original word
                                            vectors.""", default = None)
    parser.add_argument("--locTF", help = """Local TF rescaling: none, binary,
                                            sublinear (default=none).""",
                                            default="none")
    parser.add_argument("--stop", help = "Toggles stop local words stripping.",
                                                            action="store_true")
    parser.add_argument("--format", help = "The format of the embeddings file.",
                                                            default="binary")
    ...
    if args.tfidf is None and not args.pi_tfidf.startswith("1"):
    ...
    try:
        if args.format.startswith("bin"):
            embedding = load_vectors(args.embed, binary = True,
                                                        encoding = "latin-1")
        else:
            embedding = load_vectors(args.embed, binary = False,
                                                        encoding = "latin-1")
    except:
        logging.info(
            """Error while loading word embedding model. Verify if the file
            exists or if it is broken (EXIT)...\n%s\n""" % args.embed)
        exit()
# TODO: In the case this approach to work, try other pairwise metrics (sklearn)
    embedding_name = basename(args.embed).split(".")[0]
